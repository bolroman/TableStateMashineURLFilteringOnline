...
void generateStates(StateMachine *machine, ACTree *tree, int maxGotosLE, int maxGotosBM, int verbose) {
	...
	while (!nodequeue_isempty(&queue)) {
		...
		if (node->numGotos > 1) {
			...
			// DH: Add node to the node table
			machine->nodeTable[id]=node;
	...
	if (nextNode->id < 0)
	{
		while (nextNode->id < 0)
		{
			if (nextNode->match)
			{
				...
				//RB
				if(strcmp(nextNode->message,"fra")==0)
				{
					//RB
					//How to fix
					//change messages from char* to struct which will contain also an hoffman codes.
					//need to change in all places

					printf("found4 fra;ra\n");
					printf("next->numMessages - %d\n", next->numMessages);
					printf("nextNode->id - %d\n", nextNode->realID);
				}
...
void getDepths(StateMachine *machine) {
	int queue[100000];
	int in, out, size;
	int count, i, current;
	int gotos[256];
	...
	queue[in++] = 0;
	size++;
	machine->depthmap[0] = 0;

	while (size > 0) {
		current = queue[out++];
		size--;
		count = getAllNextStates(machine->states->table[current], gotos);
		for (i = 0; i < count; i++) {
			if (machine->depthmap[gotos[i]] != -1)
				continue;
			machine->depthmap[gotos[i]] = machine->depthmap[current] + 1;
			queue[in++] = gotos[i];
			size++;
		}
	}
...
void printStateMachine(StateMachine *machine) {
	...
	machine->depthmap = (int*)malloc(sizeof(int) * machine->states->size);
	for (i = 0; i < machine->states->size; i++) {
		machine->depthmap[i] = -1;
	}
	machine->depthmap[0] = 0;

	getDepths(machine);

	printf("Printing state machine:\n(-1: a pointer to depth 1, -2: a pointer to depth 2, -999: undefined field for this type of state)\n");
	printf("\nType\tID\tDepth\tGoto[PC]\tFail[LT]\t|Char (match, goto[for LT]/failure[for PC])\n");

	for (i = 0; i < machine->states->size; i++) {
		state = machine->states->table[i];
		printState(state, machine->depthmap[i]);
	}
...
StateMachine *createStateMachine(Anchor* anchorsArr, int numOfAnchors, int maxGotosLE, int maxGotosBM, int verbose) {
	...
	acBuildTreeASCII(&tree, anchorsArr, numOfAnchors, 1, 0);
