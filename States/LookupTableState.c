...
void setStateData_LT(State *state, uchar *chars, STATE_PTR_TYPE *gotos, short *ptrTypes, int *matches, STATE_PTR_TYPE failure) {
	...
	memset(&(data[current_idx_gotos]), (uchar)(-1), 256 * PTR_SIZE);
	...
	for (i = 0; i < size; i++) {
		...
		if (current_idx_gotos + (int)chars[i] * 2 + 1 >= SIZE_IN_BYTES) {
			fprintf(stderr, "[LT] Memory index out of bound: %d, mem size: %d\n", (current_idx_gotos + (int)chars[i] * 2 + 1), SIZE_IN_BYTES);
			exit(1);
		}
...
int getNextState_LT(State *lookupTableState, char *str, int length, int *idx, NextStateResult *result, StateMachine *machine, PatternTable *patternTable, int verbose) {
	...
	if (table[c * 2] != 0 || table[c * 2 + 1] != 0) {
		...
		if (verbose && result->match) {
			...
			//RB
			result->pattern.pattern = patternTable->patterns[id][count].pattern;
		}
		result->fail = FALSE;
		(*idx)++;
		return TRUE;
...
void printState_LT(State *state, int depth) {
	//StateHeader *header;
	int size;
	...
	char buff[2048];
	int nextPos;
	sprintf(buff, "LT\t%6d\t%6d\t%6d\t%6d\t", id, depth, -999, fail);
	nextPos = strlen(buff);
	...
	*/
	...
	for (i = 0; i < size; i++) {
		...
		sprintf(&(buff[nextPos]), "|%c (%d, %6d)", chars[i], matches[i], gotos[i]);
		nextPos += 14;
	}
	printf("%s\n", buff);
...
int getAllNextStates_LT(State *state, int *next) {
	int size;
	int i;
	uchar low, high;
