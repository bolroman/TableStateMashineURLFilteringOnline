	...
	do {																																	\
			...
			if ((uchar)(str)[*(idx)] != chars[i]) {																								\
				...
				if (GET_1BIT_ELEMENT(matches, i)) {																							\
					...
					if (verbose) {																											\
						...
						/*RB check if need to Add hoffman code*/																			\
		...
		if (failed) {																														\
			...
			case PTR_TYPE_REGULAR:																											\
				...
				(result)->stateID = ((pathCompressedState)[j + 1] << 8) | ((pathCompressedState)[j]); 										\
				break;																														\
			...
			case PTR_TYPE_LEVEL1:																											\
				...
				(result)->stateID = GET_FIRST_LEVEL_STATE(machine, (str)[*(idx) - 1]);														\
				break;																														\
			case PTR_TYPE_LEVEL2:																											\
				(result)->next = GET_STATE_POINTER_FROM_ID(machine, GET_SECOND_LEVEL_STATE(machine, (str)[*(idx) - 2], (str)[*(idx) - 1]));			\
				(result)->stateID = GET_SECOND_LEVEL_STATE(machine, (str)[*(idx) - 2], (str)[*(idx) - 1]);									\
				break;																														\
			default:																														\
