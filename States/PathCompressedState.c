...
int getNextState_PC(State *pathCompressedState, char *str, int slen, int *idx, NextStateResult *result, StateMachine *machine, PatternTable *patternTable, int verbose) {
		...
		if ((uchar)str[*idx] != chars[i]) {
			...
			//if (GET_MATCH_BIT(value)) {
				...
				if (verbose) {
					...
					//RB
					pattern = concat_strings_nofree(pattern, patternTable->patterns[id][count].pattern);
					memcpy(hoffmanCode,patternTable->patterns[id][count].cHoffmanCode,(patternTable->patterns[id][count].HoffmanCodeLength/8)+1);
					hoffmanCodeLength = patternTable->patterns[id][count].HoffmanCodeLength;
	...
	if (failed) {
		...
		memcpy(result->pattern.cHoffmanCode,hoffmanCode,(hoffmanCodeLength/8)+1);
		result->pattern.HoffmanCodeLength = hoffmanCodeLength;
