	...
	if (found) {
		...
		*/
		...
		if (verbose && result->match) {
			...
			memcpy(result->pattern.cHoffmanCode,patternTable->patterns[id][count].cHoffmanCode,(patternTable->patterns[id][count].HoffmanCodeLength/8)+1);
			result->pattern.HoffmanCodeLength = patternTable->patterns[id][count].HoffmanCodeLength;
