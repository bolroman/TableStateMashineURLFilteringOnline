...
#define __UINT32_MAX__ 0x0FFFFFFFF
#endif
...
void init_hash(RollingHashTool *obj) {
	...
	srand48(17);
	for (i = 0; i < 256; i++) {
		obj->byte_to_int32[i] = MAP[i];

		for (j = 0; j < 16; j++) {
			obj->tabHash1[i * 16 + j] = (unsigned int)(drand48() * __UINT32_MAX__);
			obj->tabHash2[i * 16 + j] = (unsigned int)(drand48() * __UINT32_MAX__);
			obj->tabHash3[i * 16 + j] = (unsigned int)(drand48() * __UINT32_MAX__);
		}
	}
